...
void app_callback(void *ptr) {
    ...
    if (NETSTACK_ROUTING.node_is_reachable() &&
        NETSTACK_ROUTING.get_root_ipaddr(&dest_ipaddr)) {
        /* Send to DAG root */
        LOG_INFO("Sending pressure: %d hPa request to ", pressure);
        LOG_INFO_6ADDR(&dest_ipaddr);
        LOG_INFO_("\n");
      snprintf(str, sizeof(str), "pressure: %d hPa", pressure);
      simple_udp_sendto(&udp_conn, str, strlen(str), &dest_ipaddr);
    } else {
        LOG_INFO("Not reachable yet\n");
    }
...
PROCESS_THREAD(udp_client_process, ev, data) {
    ...
    ctimer_set(&timer_app, 6 * CLOCK_SECOND + (random_rand() % (2 * CLOCK_SECOND)), app_callback, NULL);
